...
const TableTwo = ({ productList,updateProductList  }: TableTwoProps) => {
  const [country, setCountry] = useState('');
  const [showModal, setShowModal] = useState<{ isOpen: boolean; product?: Product }>({ isOpen: false });
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      {/* Map over the productList and render a row for each product */}
      {productList.map((product: any) => (
        <div
          key={product._id} // Make sure to use a unique key for each row
        ...
        >
          ...
          <div className="col-span-1 flex items-center">
            <div className="flex items-center space-x-3.5">
              <button
                ...
                onClick={() => setShowModal({ isOpen: true, product })}
              >
                <svg
              ...
              <button className="hover:text-primary" onClick={() => handleDeleteProduct(product._id)}>
                <svg
                  className="fill-current"
                  width="18"
                  height="18"
          ...
          <ModalForm isVisible={showModal.isOpen} onClose={() => setShowModal({ isOpen: false })} product={showModal.product} updateProductList={updateProductList}/>
        </div>
      ))}
